class Projects::ContributionsController < ApplicationController
  ...
  skip_before_filter :verify_authenticity_token, only: [:moip, :hook]
  ...
  def hook
    payment = Payment.new(contribution_id: params[:invoice], state: "paid", gateway: "payPal", payment_method: "payPal", value: params[:payment_gross], installments: 1, gateway_data: params)
    payment.save
    render nothing: true
  end
