    ...
    state_machine :state, initial: :draft do
      ...
      state :in_analysis, :approved, :online, :successful, :waiting_funds, :failed do
        ...
        validate do
          ...
          self.errors['rewards.size'] << "Debe haber al menos una recompensa" if self.rewards.size == 0
          self.errors['account.agency_size'] << "Agencia debe ser por lo menos 4 dígitos" if self.account && self.account.agency.size < 4
        end
      end
      ...
      state :online, :successful, :waiting_funds, :failed do
        #validates_presence_of :account, message: 'Los datos del Banco no puede estar en blanco'
        validate do
